...
FileSystemModel::FileSystemModel ()
    : AbstractItemModel(), rows_(0), columns_(DefaultColumns),// temporary value
      root_(0)
{
  root_ = new ModelNode;
  ...
  try {
    if (fs::exists(path) && fs::is_directory(path)) {
      ...
      while (it != it_end) {
        ...
        tmp->right = new ModelNode;
        tmp->right->data = RefPtr<Text>(
            new Text(fs::is_directory(it->path()) ? "d" : "-"));
        tmp->right->left = tmp->right;
        tmp = tmp->right;
        j++;
...
int FileSystemModel::GetPreferredColumnWidth (int index,
...
int FileSystemModel::GetPreferredRowHeight (int index,
                                            const ModelIndex& parent) const
{
  return BlendInt::Font::default_height();
}

bool FileSystemModel::InsertColumns (int column,
                                     int count,
...
bool FileSystemModel::RemoveColumns (int column,
                                     int count,
                                     const ModelIndex& parent)
{
  if (!parent.valid()) return false;
...
bool FileSystemModel::InsertRows (int row, int count, const ModelIndex& parent)
{
  if (!parent.valid()) return false;
...
bool FileSystemModel::RemoveRows (int row, int count, const ModelIndex& parent)
{
  if (!parent.valid()) return false;
...
ModelIndex FileSystemModel::GetIndex (int row,
                                      int column,
                                      const ModelIndex& parent) const
{
  ModelIndex index;
...
void FileSystemModel::DestroyColumnsInRow (int column,
                                           int count,
                                           ModelNode* node)
{
  DBG_ASSERT(node);
