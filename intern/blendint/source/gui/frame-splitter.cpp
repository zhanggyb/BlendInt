...
AbstractFrame* FrameSplitter::AddFrame (AbstractFrame* frame, SizePolicy policy)
{
  if ((frame == 0) || (frame->super() == this)) return 0;
...
AbstractFrame* FrameSplitter::InsertFrame (int index,
                                           AbstractFrame* frame,
                                           SizePolicy policy)
{
  if ((frame == 0) || (frame->super() == this)) return 0;
...
void FrameSplitter::PerformPositionUpdate (const AbstractView* source,
...
void FrameSplitter::PerformSizeUpdate (const AbstractView* source,
                                       const AbstractView* target,
                                       int width,
                                       int height)
{
...
Response FrameSplitter::PerformMouseMove (AbstractWindow* context)
{
  ...
  if (pressed_) {
    if (focused_frame_) response = dispatch_mouse_move(focused_frame_, context);
  } else {
