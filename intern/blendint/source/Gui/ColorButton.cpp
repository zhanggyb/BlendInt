...
namespace BlendInt {
	...
	ResponseType ColorButton::Draw (const AbstractWindow* context)
	{
		...
		glUniform2f(AbstractWindow::shaders->location(Shaders::WIDGET_OUTER_POSITION),
				0.f, 0.f);
		glUniform4fv(AbstractWindow::shaders->location(Shaders::WIDGET_OUTER_COLOR), 1, AbstractWindow::theme->regular().outline.data());
	...
	void ColorButton::InitializeColorButton ()
	{
		...
		glEnableVertexAttribArray(
				AbstractWindow::shaders->location(Shaders::WIDGET_OUTER_COORD));
