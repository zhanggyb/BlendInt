...
void Icons::CreatePixelIcons16x16 ()
{
  ...
  fs::path icon16_path(
      BLENDINT_INSTALL_PREFIX"/share/BlendInt/datafiles/blender_icons16.png");

  if (!fs::exists(icon16_path)) {
    icon16_path = fs::path(
        BLENDINT_PROJECT_SOURCE_DIR"/release/datafiles/blender_icons16.png");
  }
...
void Icons::CreatePixelIcons32x32 ()
{
  ...
  fs::path icon32_path(
      BLENDINT_INSTALL_PREFIX"/share/BlendInt/datafiles/blender_icons32.png");

  if (!fs::exists(icon32_path)) {
    icon32_path = fs::path(
        BLENDINT_PROJECT_SOURCE_DIR"/release/datafiles/blender_icons32.png");
  }
  ...
  RefPtr<IconTexture> texture(new IconTexture);
  texture->Generate(image.width(), image.height(), 32, 32, 10, 20, 10, 10);
  texture->bind();
  texture->SetWrapMode(GL_CLAMP_TO_EDGE, GL_CLAMP_TO_EDGE);
  texture->SetMinFilter(GL_LINEAR);
  texture->SetMagFilter(GL_LINEAR);
