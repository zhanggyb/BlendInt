...
namespace BlendInt {
  ...
  public:
    ...
    {
      return (left_ == left && right_ == right && top_ == top
          && bottom_ == bottom);
    }

    bool equal (const Margin& margin)
    {
      return (left_ == margin.left() && right_ == margin.right()
          && top_ == margin.top() && bottom_ == margin.bottom());
    }
  ...
  {
    return (src.left_ == dst.left_) && (src.right_ == dst.right_)
        && (src.top_ == dst.top_) && (src.bottom_ == dst.bottom_);
  }
