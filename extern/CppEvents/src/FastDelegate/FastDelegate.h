...
template<		int N>
		...
		struct SimplifyMemFunc<SINGLE_MEMFUNCPTR_SIZE>
		{
			...
			inline static GenericClass * Convert (X *pthis,
			        XFuncType function_to_bind, GenericMemFuncType &bound_func)
			{
#if defined __DMC__  
...
#else 
				bound_func =
				        reinterpret_cast<GenericMemFuncType>(function_to_bind);
#endif
...
namespace detail {
	...
	public:
		...
		{
			m_pthis = SimplifyMemFunc<sizeof(function_to_bind)>::Convert(pthis,
			        function_to_bind, m_pFunction);
#if !defined(FASTDELEGATE_USESTATICFUNCTIONHACK) && !defined(FASTDELEGATE_NOSUPPORTFORSTATICFUNCTIONS)
			m_pStaticFunction = 0;
#endif
		}
...
#else // !defined(FASTDELEGATE_NOSUPPORTFORSTATICFUNCTIONS)
		template<class DerivedClass>
...
public:
	...
	inline void bind (Y *pthis,
	        DesiredRetType (X::*function_to_bind) (Param1 p1))
	{
		m_Closure.bindmemfunc(detail::implicit_cast<X*>(pthis),
		        function_to_bind);
	}
...
public:
	...
	FastDelegate2 (const Y *pthis,
	        DesiredRetType (X::*function_to_bind) (Param1 p1, Param2 p2) const)
	{
		m_Closure.bindconstmemfunc(detail::implicit_cast<const X*>(pthis),
		        function_to_bind);
	}
...
public:
	...
	FastDelegate5 (const Y *pthis,
	        DesiredRetType (X::*function_to_bind) (Param1 p1, Param2 p2,
	                Param3 p3, Param4 p4, Param5 p5) const)
	{
		m_Closure.bindconstmemfunc(detail::implicit_cast<const X*>(pthis),
		        function_to_bind);
	}
...
class FastDelegate<
        R (Param1, Param2, Param3, Param4, Param5, Param6, Param7, Param8)>
// Inherit from FastDelegate8 so that it can be treated just like a FastDelegate8
: public FastDelegate8<Param1, Param2, Param3, Param4, Param5, Param6, Param7,
        Param8, R>
{
public:
	// Make using the base type a bit easier via typedef.
...
FastDelegate1<Param1, FASTDLGT_RETTYPE> MakeDelegate (Y* x,
...
FastDelegate7<Param1, Param2, Param3, Param4, Param5, Param6, Param7,
        FASTDLGT_RETTYPE> MakeDelegate (Y* x,
        RetType (X::*func) (Param1 p1, Param2 p2, Param3 p3, Param4 p4,
