...
namespace BlendInt {
	...
	void GridFloor::Render (const glm::mat4& projection_matrix, const glm::mat4& view_matrix)
	{
		...
		RefPtr<GLSLProgram> program = shaders()->primitive_program();
		program->use();

		glUniformMatrix4fv(shaders()->location(Shaders::PRIMITIVE_PROJECTION), 1, GL_FALSE, glm::value_ptr(projection_matrix));
		glUniformMatrix4fv(shaders()->location(Shaders::PRIMITIVE_VIEW), 1, GL_FALSE, glm::value_ptr(view_matrix));
		glUniformMatrix4fv(shaders()->location(Shaders::PRIMITIVE_MODEL), 1, GL_FALSE, glm::value_ptr(glm::mat4(1.0)));

		if(vaos_[0] != 0) {
			glVertexAttrib4f(shaders()->location(Shaders::PRIMITIVE_COLOR), 0.35f, 0.35f, 0.35f, 1.f);
			glBindVertexArray(vaos_[0]);
			glDrawArrays(GL_LINES, 0, total_count);
		}

		if(vaos_[1] != 0) {	// show x axis
			glVertexAttrib4f(shaders()->location(Shaders::PRIMITIVE_COLOR), 1.f, 0.f, 0.f, 0.5f);
			glBindVertexArray(vaos_[1]);
			glDrawArrays(GL_LINES, 0, 2);
		}

		if(vaos_[2] != 0) {	// show y axis
			glVertexAttrib4f(shaders()->location(Shaders::PRIMITIVE_COLOR), 0.f, 1.f, 0.f, 0.5f);
			glBindVertexArray(vaos_[2]);
			glDrawArrays(GL_LINES, 0, 2);
		}

		if(vaos_[3] != 0) {	// show z axis
			glVertexAttrib4f(shaders()->location(Shaders::PRIMITIVE_COLOR), 0.f, 0.f, 1.f, 0.5f);
	...
	void GridFloor::InitializeGrid()
	{
		...
		glEnableVertexAttribArray(shaders()->location(Shaders::PRIMITIVE_COORD));
		glVertexAttribPointer(shaders()->location(Shaders::PRIMITIVE_COORD), 3, GL_FLOAT, GL_FALSE, 0, 0);
