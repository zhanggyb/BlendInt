...
FileBrowser::FileBrowser ()
    : AbstractItemView(), history_index_(0), highlight_index_(-1)
{
  set_size(400, 300);
...
bool FileBrowser::OpenParent ()
{
  ...
  if (!history_.empty()) {
    ...
    if (history_index_ == 0) {
      ...
      if (fs::exists(p)) history_.push_front(p.native());
...
{
  RefPtr<FileSystemModel> fs_model = RefPtr<FileSystemModel>::cast_dynamic(
      model);
...
void FileBrowser::PerformSizeUpdate (const AbstractView* source,
