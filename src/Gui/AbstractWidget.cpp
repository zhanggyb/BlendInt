...
namespace BlendInt {
	...
	AbstractWidgetExt* AbstractWidgetExt::focused_widget = 0;

	AbstractWidgetExt::AbstractWidgetExt ()
	: AbstractExtraForm(),
	m_z(0),
	m_container(0)
	{
		m_events.reset(new Cpp::ConnectionScope);
		m_flag.set(WidgetFlagVisibilityExt);
	}
	...
	void AbstractWidgetExt::Resize (unsigned int width, unsigned int height)
	{
	...
	void AbstractWidgetExt::Resize (const Size& size)
	{
		// If the object is managed by a layout, disallow position setting
		if(locked()) return;

		if(AbstractWidgetExt::size() == size) return;
	...
	void AbstractWidgetExt::SetPosition (int x, int y)
	{
		// If the object is managed by a layout, disallow position setting
		if(locked()) return;
	...
	void AbstractWidgetExt::SetPosition (const Point& pos)
	{
		// If the object is managed by a layout, disallow position setting
		if(locked()) return;
	...
	void AbstractWidgetExt::SetPreferredSize(unsigned int width, unsigned int height)
	{
	...
	void AbstractWidgetExt::SetPreferredSize(const Size& size)
	{
	...
	void AbstractWidgetExt::SetMinimalSize(unsigned int width, unsigned int height)
	{
	...
	void AbstractWidgetExt::SetMinimalSize(const Size& size)
	{
	...
	void AbstractWidgetExt::SetMaximalSize(unsigned int width, unsigned int height)
	{
		if(width < preferred_size().width() ||
				height < preferred_size().height())
		return;
	...
	void AbstractWidgetExt::SetMaximalSize(const Size& size)
	{
		if(size.width() < preferred_size().width() ||
				size.height() < preferred_size().height())
		return;
	...
	void AbstractWidgetExt::SetLayer (int z)
	{
		// If the object is managed by a layout, disallow position setting
		if(locked()) return;
		...
		if(m_flag[WidgetFlagInContextManagerExt]) {
	...
	void AbstractWidgetExt::SetVisible (bool visible)
	{
		if(m_flag[WidgetFlagVisibilityExt] == visible)
			return;

		if(Update (UpdateRequest(Predefined, WidgetVisibility, &visible))) {
			m_flag[WidgetFlagVisibilityExt] = visible ? 1 : 0;
			fire_property_changed_event(WidgetVisibility);
		}
	}

	void AbstractWidgetExt::Refresh()
	{
	...
	void AbstractWidgetExt::RenderToTexture (size_t border, GLTexture2D* texture)
	{
		if(!texture) return;
	...
	void AbstractWidgetExt::RenderToFile(const char* filename, unsigned int border)
	{
		GLsizei width = size().width() + border * 2;
		GLsizei height = size().height() + border * 2;
	...
	void AbstractWidgetExt::SetPosition(AbstractWidgetExt* obj, int x, int y)
	{
	...
	void AbstractWidgetExt::SetPosition(AbstractWidgetExt* obj, const Point& pos)
	{
	...
	void AbstractWidgetExt::Resize (AbstractWidgetExt* obj, unsigned int width, unsigned int height)
	{
	...
	void AbstractWidgetExt::Resize (AbstractWidgetExt* obj, const Size& size)
	{
	...
	void AbstractWidgetExt::DispatchRender(AbstractWidgetExt* other)
	{
		//other->Draw();
	}

	ResponseType AbstractWidgetExt::dispatch_key_press_event (AbstractWidgetExt* obj,
			const KeyEvent& event)
	{
		return obj->KeyPressEvent(event);
	}

	ResponseType AbstractWidgetExt::dispatch_mouse_move_event (AbstractWidgetExt* obj,
			const MouseEvent& event)
	{
		return obj->MouseMoveEvent(event);
	}

	ResponseType AbstractWidgetExt::dispatch_mouse_press_event (AbstractWidgetExt* obj,
			const MouseEvent& event)
	{
		return obj->MousePressEvent(event);
	}

	ResponseType AbstractWidgetExt::dispatch_mouse_release_event (AbstractWidgetExt* obj,
			const MouseEvent& event)
	{
		return obj->MouseReleaseEvent(event);
	}

	bool AbstractWidgetExt::CompositeToScreenBuffer (GLTexture2D* tex,
			unsigned int border)
	{
		bool result = false;
