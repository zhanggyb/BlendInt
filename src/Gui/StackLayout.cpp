...
namespace BlendInt {
	...
	void StackLayout::PushBack (AbstractWidget* widget)
	{
	...
	AbstractWidget* StackLayout::GetWidget (int index) const
	{
		if(index > (widget_count() - 1)) return 0;

		return GetWidgetAt(index);
	}

	void StackLayout::HideSubWidget(int index)
	{
	...
	bool StackLayout::SizeUpdateTest (const SizeUpdateRequest& request)
	{
	...
	bool StackLayout::PositionUpdateTest (const PositionUpdateRequest& request)
	{
		if(request.source()->container() == this) {
			return false;
		}
	...
	ResponseType StackLayout::Draw (Profile& event)
	{
		return Ignore;
	}
	...
	ResponseType StackLayout::ContextMenuPressEvent(const ContextMenuEvent& event)
	{
		return Ignore;
	}

	ResponseType StackLayout::ContextMenuReleaseEvent(const ContextMenuEvent& event)
	{
		return Ignore;
	}

	ResponseType StackLayout::MousePressEvent(const MouseEvent& event)
	{
		return Ignore;
	}

	ResponseType StackLayout::MouseReleaseEvent(const MouseEvent& event)
	{
		return Ignore;
	}

	ResponseType StackLayout::MouseMoveEvent(const MouseEvent& event)
	{
		return Ignore;
	}
