...
namespace BlendInt {
	...
	ToolBarExt::~ToolBarExt ()
	{
		...
		if(focused_widget_) {
			...
			focused_widget_->destroyed().disconnectOne(this, &ToolBarExt::OnFocusedWidgetDestroyed);
			focused_widget_ = 0;
		}

		if(hovered_widget_) {
			hovered_widget_->destroyed().disconnectOne(this, &ToolBarExt::OnHoverWidgetDestroyed);
			ClearHoverWidgets(hovered_widget_);
		}
	}

	void ToolBarExt::Add(Widget* widget, bool append)
	{
	...
	bool ToolBarExt::IsExpandX() const
	{
		return true;
	}

	Size ToolBarExt::GetPreferredSize() const
	{
		Size preferred_size;
		...
		if(first_child() == 0) {
			...
			preferred_size.set_height(max_font_height + margin_.vsum());
	...
	bool ToolBarExt::SizeUpdateTest(const SizeUpdateRequest& request)
	{
		return true;
	}

	void ToolBarExt::PerformPositionUpdate(const PositionUpdateRequest& request)
	{
	...
	void ToolBarExt::PerformSizeUpdate(const SizeUpdateRequest& request)
	{
		if(request.target() == this) {
	...
	bool ToolBarExt::PreDraw(Profile& profile)
	{
	...
	ResponseType ToolBarExt::Draw(Profile& profile)
	{
		for(AbstractWidget* p = first_child(); p; p = p->next()) {
	...
	void ToolBarExt::PostDraw(Profile& profile)
	{
		glDisable(GL_SCISSOR_TEST);
		glViewport(0, 0, profile.context()->size().width(), profile.context()->size().height());
	}

	void ToolBarExt::FocusEvent(bool focus)
	{
	}

	void ToolBarExt::MouseHoverInEvent(const MouseEvent& event)
	{
	}

	void ToolBarExt::MouseHoverOutEvent(const MouseEvent& event)
	{
		if(hovered_widget_) {
			hovered_widget_->destroyed().disconnectOne(this, &ToolBarExt::OnHoverWidgetDestroyed);
			ClearHoverWidgets(hovered_widget_);
	...
	ResponseType ToolBarExt::KeyPressEvent(const KeyEvent& event)
	{
	...
	ResponseType ToolBarExt::MousePressEvent(const MouseEvent& event)
	{
	...
	ResponseType ToolBarExt::MouseReleaseEvent(const MouseEvent& event)
	{
		ResponseType retval = Ignore;
	...
	ResponseType ToolBarExt::MouseMoveEvent(const MouseEvent& event)
	{
		ResponseType retval = Ignore;
	...
	ResponseType ToolBarExt::DispatchHoverEvent(const MouseEvent& event)
	{
			...
			if(new_hovered_widget != hovered_widget_) {
				...
				if(hovered_widget_) {
					hovered_widget_->destroyed().disconnectOne(this,
							&ToolBarExt::OnHoverWidgetDestroyed);
				}
	...
	void ToolBarExt::FillSubWidgets()
	{
		int x = margin_.left();
		int y = margin_.bottom();
	...
	int ToolBarExt::GetLastPosition() const
	{
		int x = margin_.left();

		if (last_child()) {
			x = last_child()->position().x() + last_child()->size().width() + space_;
		}

		return x;
	}

	void ToolBarExt::SetFocusedWidget(Widget* widget)
	{
		...
		if (focused_widget_) {
			...
			events()->connect(focused_widget_->destroyed(), this, &ToolBarExt::OnFocusedWidgetDestroyed);
		}
	}

	void ToolBarExt::OnFocusedWidgetDestroyed(Widget* widget)
	{
		...
		widget->destroyed().disconnectOne(this, &ToolBarExt::OnFocusedWidgetDestroyed);

		focused_widget_ = 0;
	}

	void ToolBarExt::OnHoverWidgetDestroyed(Widget* widget)
	{
		assert(widget->hover());
		assert(hovered_widget_ == widget);

		DBG_PRINT_MSG("unset hover status of widget %s", widget->name().c_str());
		widget->destroyed().disconnectOne(this, &ToolBarExt::OnHoverWidgetDestroyed);
